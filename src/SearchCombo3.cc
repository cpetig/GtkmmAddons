...
struct Gtk::SearchCombo3::Model : Gtk::ListStore
...
Gtk::SearchCombo3::Model::Model(const TreeModelColumnRecord& columns, Gtk::ComboBoxEntry *e)
...
void Gtk::SearchCombo3::Model::clear_list()
{ stop_if_running();
  Gtk::ListStore::clear();
//  combo->popdown();
...
void Gtk::SearchCombo3::Model::fill_list_if_necessary()
{ if (!search_in_progress && !search_finished)
    fill_list();
}

void Gtk::SearchCombo3::Model::fill_list()
...
void Gtk::SearchCombo3::Model::entry_changed()
{ // narrow optimization (gtksearchcombo.c:712)
  fill_list();
//  combo->get_entry()->grab_focus();
}

bool Gtk::SearchCombo3::Model::close_search()
...
bool Gtk::SearchCombo3::Model::fill_idle()
{ gboolean continue_=false;
...
void Gtk::SearchCombo3::Model::stop_if_running()
...
Glib::RefPtr<Gtk::SearchCombo3::Model> Gtk::SearchCombo3::Model::create(const TreeModelColumnRecord& columns, Gtk::ComboBoxEntry *e)
...
bool Gtk::SearchCombo3::popup_key_pr(GdkEventKey* k)
{ return on_key_press_event(k);
}

bool Gtk::SearchCombo3::popup_key_rel(GdkEventKey* k)
{ return on_key_release_event(k);
}

void Gtk::SearchCombo3::init()
{ mymodel=Model::create(m_text_columns,this);
  set_model(mymodel);
  set_text_column(m_text_columns.m_column);
  get_entry()->signal_changed().connect(sigc::mem_fun(*this,&SearchCombo3::on_entry_changed));
  signal_changed().connect(signal_activate().make_slot());
  get_entry()->signal_activate().connect(signal_activate().make_slot());
//  sigc::mem_fun(*this,&SearchCombo3::on_changed));
...
//  std::cerr << "appears-as-list " << Glib::PropertyProxy<bool>(this,"appears-as-list").get_value() << '\n';
  ...
  Glib::wrap(GTK_WINDOW(GTK_COMBO_BOX(gobj())->priv->popup_window))->signal_key_press_event()
    .connect(sigc::mem_fun(*this,&SearchCombo3::popup_key_pr));
  Glib::wrap(GTK_WINDOW(GTK_COMBO_BOX(gobj())->priv->popup_window))->signal_key_release_event()
    .connect(sigc::mem_fun(*this,&SearchCombo3::popup_key_rel));
#endif
  // ENDHACK
}

Gtk::SearchCombo3::SearchCombo3(bool always_fill, bool autoexpand)
{ init();
}

#if 0
Gtk::SearchCombo3::SearchCombo3(TextModelColumns const& col)
  : m_text_columns(col)
{ init();
}
#endif
...
sigc::signal0<void> &Gtk::SearchCombo3::signal_activate()
{ return activate;
}

sigc::signal2<void,gboolean *,GtkSCContext> &Gtk::SearchCombo3::signal_search()
...
Gtk::TreeModel::iterator Gtk::SearchCombo3::add_item(const Glib::ustring &text)
